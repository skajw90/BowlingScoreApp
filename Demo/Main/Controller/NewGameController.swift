...
class NewGameController: NewGameViewDataSource, NewGameViewDelegate {
    ...
    func setSelectedFrame(index: Int) {
        ...
        if index == 1 && selectedFrame < curFrame && gameScore.getInput(index: selectedFrame)! == 11 && turn == 0 && selectedFrame < 18 { moveTo = 2 }
    ...
    func setScore(score: Int) {
        ...
        if turn == 0 && score == 11 && selectedFrame < 18 {
            if selectedFrame <= curFrame { gameScore.setInput(index: selectedFrame + 1, score: nil) }
            index = 2
        }
    ...
    private func checkAvailability(index: Int) -> Bool {
        ...
        if index == 1 && (selectedFrame == 20 || selectedFrame > curFrame || (selectedFrame == 19 && gameScore.getInput(index: selectedFrame) != nil && gameScore.getInput(index: selectedFrame) != 11) || (selectedFrame == 19 && gameScore.getInput(index: selectedFrame - 1)! != 11)) { return false }
        if index == -1 && selectedFrame <= 0 { return false }
        return true
    }
    private func update() { newGameView.scoreFrameView.update() }
    private func highlightCurrentPos(prev: Int?, prevTurn: Int, cur: Int) {
        if let prev = prev { newGameView.scoreFrameView.frameViews[prev].scoreInputView.highlightBorder(turn: prevTurn, on: false) }
