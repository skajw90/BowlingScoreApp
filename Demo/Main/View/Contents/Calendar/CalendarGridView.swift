...
class CalendarGridView: UIView {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if firstTouchedCell == endLocation {
            if let selectedCell = selectedCell {
                ...
                else { self.selectedCell = firstTouchedCell }
            }
            else { self.selectedCell = firstTouchedCell }
            updateCells()
        }
    ...
    // MARK: - Helper Method to initialize Cells
    func initializeCells() {
        for cell in cells { cell.removeFromSuperview() }
        cells = []
    }
    
    // MARK: - Update Cells without params, Default
    func updateCells() { updateCells(map: map) }
    
    // MARK: - Update Cells with params
    func updateCells(map: [Int]) {
        ...
        for y in 0 ..< 6 {
            for x in 0 ..< 7 {
                ...
                cell.x = x
                cell.y = y
                if map[7 * y + x] == 1 && !isMain { isMain = true }
                else if isMain && map[7 * y + x] == 1 { isMain = false }
                cell.isMainDaysInMonth = isMain
                if isMain {
                    if dataSource!.hasContent(index: map[7 * y + x]) { cell.hasContent = true }
                    else { cell.hasContent = false }
                }
                else { cell.hasContent = false }
        ...
        else if let cell = todayCell { cell.isToday = false }
        if let selectedCell = selectedCell {
    ...
    // MARK: - Helper Method to get touch info
    func getTouchedCell(location: CGPoint) -> Int { return Int(location.x / (bounds.maxX / 7)) + Int(location.y / (bounds.maxY / 6)) * 7 }
}
